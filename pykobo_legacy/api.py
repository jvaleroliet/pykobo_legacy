...
def search_select(search, token=token, kc_server_url=server_url, id=False):
    ...
    forms_match = forms.loc[forms["title"].str.lower(
...
def create_dfs(form, language=None):
    ...
    survey[["type_nochoice", "list_name"]
           ] = survey["type"].str.split(" ", expand=True)

    # Procesamos los datos de selección única
    # Separamos los choices de selección única
    choices_so = copy(choices.loc[choices["list_name"].isin(
        survey.loc[survey["type_nochoice"] == "select_one"]["list_name"])])
    ...
    coord_max = 0
    # Como la tabla original de choices se va a desordenar, tenemos que crear
    # también una que va a ser copia de esta donde meteremos los datos en el
    # nuevo orden
    ...
    for lista in set(choices_so["list_name"]):
        ...
        df_aux_lista = pd.DataFrame({"list_name": [lista], "coord_min": [
                                    coord_min], "coord_max": [coord_max]})
        aux = pd.concat([aux, df_aux_lista], ignore_index=True)

    # Porcesamos ahora los datos de las listas de selección multiple. El único
    # cambio es que tenemos que utilizar las funciones definidas al principio
    # del script para crear las combinaciones
    choices_sm = copy(choices.loc[choices["list_name"].isin(
        survey.loc[survey["type_nochoice"] == "select_multiple"]["list_name"])])
    ...
    for lista in set(choices_sm["list_name"]):
        ...
        d = {
            "name": process_choices(
                df_or_lista["name"].tolist()), label: process_choices(
                df_or_lista[label].tolist(), True)}
...
class Form:
    def __init__(self, id, token=token, kc_server_url=server_url):
        ...
        self.choices = get_xls(
            self.id,
            self.token,
            self.kc_server_url,
            sheet=2)
        self.settings = get_xls(
            self.id,
            self.token,
            self.kc_server_url,
            sheet=3)
        self.data = json_to_df(self.id, self.token, self.kc_server_url)
        self.media = media.get_media_form(
            self.id, self.token, self.kc_server_url)
    ...
    def get_data_labels(self, choices=False, survey=False, language=None):
        ...
        if choices:
            ...
            survey_copy[["type_nochoice", "list_name"]
                        ] = survey_copy["type"].str.split(" ", expand=True)
            survey_subset = copy(
                survey_copy[["name", "type_nochoice", "list_name"]])
            survey_subset = survey_subset.loc[survey_subset["type_nochoice"].isin(
                ["select_one", "select_multiple"])]
            choices_copy = copy(self.choices)
            if not survey_subset.loc[survey_subset["type_nochoice"].isin(
                    ["select_multiple"])].empty:
                aux, choices_b, survey_b = create_dfs(self, language)
                choices_copy = copy(choices_b)

            keys = pd.merge(survey_subset[["list_name",
                                           "name"]],
                            choices_copy,
                            on="list_name",
                            how="left",
            ...
            for col in list(set(survey_subset["name"])):
                try:
                    ...
                    keys_subset = keys_subset[[
                        "name_survey", "name_choices", label]]
                    col_copy = pd.merge(
                        col_copy,
                        keys_subset,
                        left_on=col,
                        right_on="name_choices",
                        how="left")
        ...
        if survey:
            # Esto no funciona bien, hay que hacer algo con las columnas con
            # labels repetidas
            column_names = pd.DataFrame(
                columns=["Colnames"], data=self.data.columns)
            column_names["Nogroup"] = column_names["Colnames"].apply(
                lambda x: x.split("/")[-1])
            column_names = pd.merge(column_names, self.survey[[
                                    "name", label]], left_on="Nogroup", right_on="name", how="left")
            column_names["Final"] = column_names[["Colnames", label]].apply(
                lambda x: x[1] if pd.notna(x[1]) else x[0], axis=1)
