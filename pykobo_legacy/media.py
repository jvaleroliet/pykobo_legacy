...
def upload_media(
    ...
    if file_extension not in valid_media:
        raise ValueError(
            "upload_media: media file must be one of %r." %
            valid_media)
            ...
            if rewrite:
                ...
                while response.status_code != 204:
                    response = requests.delete(
                        "{}/metadata/{}".format(x, del_id), headers=headers)
                    print(response.status_code)
                    time.sleep(1)
                break
            else:
                raise ValueError(
                    "There is already a file with the same name! Select new name or set rewrite = True")

    response = requests.post(
        "{}/metadata.json".format(x),
        data=data,
        files=files,
        headers=headers)
...
def upload_media_direct(
    ...
    if file_extension not in valid_media:
        raise ValueError(
            "upload_media: semestre must be one of %r." %
            valid_media)
    ...
    for each in dict_response:
        if each['xform'] == int(
                idform) and each['data_value'] == file_name_new:
            if rewrite:
                del_id = each['id']
                # print("{}/metadata/{}".format(x, del_id))
                response.status_code = 403
